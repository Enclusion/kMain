...
public class EconomyManager implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (cmd.equalsIgnoreCase("buy")) {
            if (args.length > 0 && args.length < 3) {
                ...
                MessageManager.sendMessage(p, "&7You have purchased &c" + amount + " &7of &c" + item.getName() + " &7for &c$" + formatDouble((item.getPrice() * amount)) + "&7.");
                addItem(p, stack);
            } else {
        ...
        if (cmd.equalsIgnoreCase("price")) {
            if (args.length == 1) {
                ...
                MessageManager.sendMessage(p, "&7The item &c" + item.getName() + " &7costs &c$" + formatDouble(item.getPrice()) + "&7.");
                return true;
                ...
                MessageManager.sendMessage(p, "&c" + amount + " &7of &c" + item.getName() + " &7costs &c$" + formatDouble(item.getPrice() * amount) + "&7.");
                return true;
            } else {
            ...
            if (args.length == 2) {
                if (Bukkit.getPlayer(args[0]) != null) {
                    ...
                    try {
                        ...
                        MessageManager.sendMessage(p, "&7You have paid &c" + t.getName() + " &c$" + formatDouble(money.doubleValue()) + "&7.");
                        MessageManager.sendMessage(t, "&7You have been paid &c" + formatDouble(money.doubleValue()) + " &7by &c" + p.getName() + "&7.");
    ...
    public void loadShopGUI() {
        ...
        itemInventory2.setItem(45, new ItemBuilder(Material.STAINED_GLASS_PANE).durability(14).name("&cPrevious Page").build());
        itemInventory2.setItem(53, new ItemBuilder(Material.STAINED_GLASS_PANE).durability(5).name("&aNext Page").build());
        itemInventory3.setItem(45, new ItemBuilder(Material.STAINED_GLASS_PANE).durability(14).name("&cPrevious Page").build());
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (i <= 44) {
                itemInventory1.addItem(new ItemBuilder(Material.getMaterial(item.getId())).durability((item.damage != -1 ? item.damage : 0)).amount(item.getAmount())
                        .name("&c" + item.getName())
                        .addLore("&7Price: &c$" + formatDouble(item.getPrice()))
                        .addLore("&7Worth: &c$" + formatDouble(getPrice(new ItemStack(item.id, item.getAmount(), (short) (item.damage != -1 ? item.damage : 0)), item.getAmount())))
                        .build());
            }
            if (i > 44 && i <= 88) {
            ...
            if (i > 88 && i <= 132) {
                itemInventory3.addItem(new ItemBuilder(Material.getMaterial(item.getId())).durability((item.damage != -1 ? item.damage : 0)).amount(item.getAmount())
                        .name("&c" + item.getName())
                        .addLore("&7Price: &c$" + formatDouble(item.getPrice()))
                        .addLore("&7Worth: &c$" + formatDouble(getPrice(new ItemStack(item.id, item.getAmount(), (short) (item.damage != -1 ? item.damage : 0)), item.getAmount())))
                        .build());
    ...
    public boolean sellAllItemInHand(Player player) {
        ...
        MessageManager.sendMessage(player, "&7You got &c$" + formatDouble(price) + " &7for selling &c" + amount + " " + itemText + "&7.");
