...
public class BaseTeamCommand extends BaseCommand {
    ...
    List<String> list = Arrays.asList("create", "join", "leave", "info", "roster", "chat", "hq", "rally", "ff",
	    "password", "kick", "promote", "demote", "sethq", "setrally", "list");

    String[] teamUsage = { "&7*** &3Members &7***", "&7/team create <name> <password> &3- Create a new team",
	    "&7/team join <name> <password> &3- Join a team", "&7/team leave &3- Leave your current team",
	    "&7/team info <player> &3- Shows information about a player's team",
	    "&7/team roster <team> &3- Shows information about a given team", "&7/team chat &3- Toggle team chat mode",
	    "&7/team hq &3- Teleports you to your team's headquarters",
	    "&7/team rally &3- Teleports you to your team's rally point",
	    "&7/team list &3- Shows the top 5 tteams by players.", "&7*** &3Managers Only &7***",
	    ...
	    "&7/team demote <player> &3- Demote a player to member", "&7/team sethq &3- Sets the team's headquarters",
	...
	if (args.length == 0) {
		...
		MessageManager.sendMessage(sender,
			"&cAn unexpected error occured: " + ex.getLocalizedMessage() + "\nContact an admin!");
		ex.printStackTrace();
	    }
	}
    }
	...
	if (sender instanceof Player) {
		...
		if (args[0].equalsIgnoreCase("friendlyfire") || args[0].equalsIgnoreCase("ff")
		    ...
		    List<String> list2return = options.stream().filter(opt -> opt.toLowerCase().startsWith(args[1]))
			    .collect(Collectors.toList());
		...
		if (args[0].equalsIgnoreCase("i") || args[0].equalsIgnoreCase("info")) {
		    List<String> list2return = PlayerUtility.toList(PlayerUtility.getOnlinePlayers()).stream()
			    .filter(opt -> opt.toLowerCase().startsWith(args[1])).collect(Collectors.toList());
		    Collections.sort(list2return);
		    return list2return;
		}
		...
		if (args[0].equalsIgnoreCase("r") || args[0].equalsIgnoreCase("roster")) {
		    List<String> list2return = tm.getTeams().stream()
			    .filter(opt -> opt.getName().toLowerCase().startsWith(args[1])).map(Team::getName)
			    .collect(Collectors.toList());
		    Collections.sort(list2return);
		    return list2return;
		}
