...
public class IronBoatListener implements Listener {
	...
	ShapedRecipe s1 = new ShapedRecipe(ironboat).shape("III", "IMI", "III").setIngredient('I', Material.IRON_INGOT)
		.setIngredient('M', Material.MINECART);
		...
		if (e.getItem() != null) {
			...
			if (e.getItem().hasItemMeta()) {
				...
				if (e.getItem().getItemMeta().getDisplayName().contains(ChatColor.GRAY + "Iron Boat")) {
				    ...
				    Minecart cart = block.getWorld().spawn(block.getLocation().add(0.0D, 2.0D, 0.0D),
					    Minecart.class);
				...
				if (e.getItem() != null) {
					...
					if (e.getItem().hasItemMeta()) {
						...
						if (e.getItem().getItemMeta().getDisplayName()
							.contains(ChatColor.GRAY + "Iron Boat")) {
						    e.setUseItemInHand(Event.Result.DENY);
						    e.setCancelled(true);

						    Minecart cart = b.getWorld().spawn(
							    b.getLocation().add(0.0D, 2.0D, 0.0D), Minecart.class);
						    cart.setMetadata("IronBoat",
							    new FixedMetadataValue(Legacy.getInstance(), true));
	...
	if (e.getVehicle() instanceof Minecart) {
		...
		if (e.getVehicle().getLocation().subtract(0.0D, 1.0D, 0.0D).getBlock().getType() == Material.LAVA
			|| e.getVehicle().getLocation().subtract(0.0D, 1.0D, 0.0D).getBlock()
				.getType() == Material.STATIONARY_LAVA) {
		    Minecart cart = (Minecart) e.getVehicle();
		    Player p = (Player) e.getVehicle().getPassenger();

		    Vector vect = new Vector(p.getLocation().getDirection().getX(),
			    cart.getLocation().getDirection().getY(), p.getLocation().getDirection().getZ());
	...
	if (e.getVehicle().hasMetadata("IronBoat")) {
	    e.setCancelled(true);
	    e.getVehicle().getLocation().getWorld().dropItemNaturally(e.getVehicle().getLocation(),
		    new ItemBuilder(Material.MINECART).name("&7Iron Boat").build());
	    e.getVehicle().remove();
	}
    }
}
