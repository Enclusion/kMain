...
public class BlockUtils {
	...
	loc.getWorld().playEffect(loc, Effect.STEP_SOUND,
		type == Material.AIR ? block.getType().getId() : type.getId());
	block.setType(type);
	block.setData(data);
	block.getLocation().getWorld().playEffect(block.getLocation(), Effect.STEP_SOUND,
		block.getState().getBlock().getType() == Material.AIR ? block.getState().getBlock().getType().getId()
			: block.getState().getBlock().getType().getId());
    }

    public static List<Location> circle(final Location loc, final int radius, final int height, final boolean hollow,
    ...
    public static List<Block> getRelativeBlocks(final Location center, final int radius,
	    final Collection<Block> startBlocks, final Set<Material> types) {
