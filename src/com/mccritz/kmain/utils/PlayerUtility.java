...
public class PlayerUtility {
    ...
    public static void updateScoreboard(Player p) {
        ...
        if (Command_hud.getDisplayList().contains(p.getUniqueId())) {
            if (scoreboard != null) {
                if (scoreboard.getEntry("balance") != null) {
                    scoreboard.getEntry("balance")
                            .setText("&7Balance: &a" + Legacy.getInstance().getEconomyManager().formatDouble(Legacy.getInstance().getProfileManager().getProfile(p.getUniqueId()).getBalance()))
                            .send();
                } else {
