package com.mccritz.kmain.utils.glaedr.scoreboards;
...
import com.mccritz.kmain.utils.glaedr.events.EntryCancelEvent;
import com.mccritz.kmain.utils.glaedr.events.EntryFinishEvent;
import com.mccritz.kmain.utils.glaedr.events.EntryPauseEvent;
import com.mccritz.kmain.utils.glaedr.events.EntryTickEvent;
...
public class Entry {
    ...
    /**
     * @param id
     *            Unique string to identify and pull an entry after it is
     *            created
     * @param playerScoreboard
     *            A player's scoreboard, which you can get by calling
    ...
    /**
     * @param text
     *            Will be used to display on scoreboard and inserted before time
     *            if applicable
     * @return Entry
     */
    ...
    /**
     * @param countup
     *            Boolean that will set countup to
     * @return Entry
     */
    ...
    /**
     * @param countdown
     *            Whether entry will be static or countdown, time cannot be null
     * @return Entry
     */
    ...
    /**
     * @param time
     *            Countdown time, countdown must be true
     * @return Entry
     */
    ...
    /**
     * Sets up team information, and sends Entry to PlayerScoreboard, should be
     * called after all other values are set
     *
     * @return Entry
     */
    public Entry send() {
	...
	if (!countdown) {
		...
		task = new BukkitRunnable() {
			...
			if (time.doubleValue() <= 60) {
			    ...
			    String newText = text + " " + formatter.format(minutes) + ":" + formatter.format(seconds)
				    + "m";
			    ...
			    int seconds = time.intValue() % 60;
			    DecimalFormat formatter = new DecimalFormat("00");
			    String newText = text + " " + formatter.format(hours) + ":" + formatter.format(minutes)
				    + ":" + formatter.format(seconds) + "h";
			    textTime = formatter.format(hours) + ":" + formatter.format(minutes) + ":"
				    + formatter.format(seconds) + "h";
			    minCount++;
