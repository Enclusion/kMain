...
public class PotionEffectSerialization {
	...
	for (int i = 0; i < effs.length; i++) {
	    ...
	    if (!NumberUtility.isNumber(effect[0]))
		throw new IllegalArgumentException(
			serializedEffects + " - PotionEffect " + i + " (" + effs[i] + "): id is not an integer");
	    if (!NumberUtility.isNumber(effect[1]))
		throw new IllegalArgumentException(
			serializedEffects + " - PotionEffect " + i + " (" + effs[i] + "): duration is not an integer");
	    if (!NumberUtility.isNumber(effect[2]))
		throw new IllegalArgumentException(
			serializedEffects + " - PotionEffect " + i + " (" + effs[i] + "): amplifier is not an integer");
