...
public class Kit {
	...
	if (nextUse == 0L) {
	    ...
	    MessageManager.sendMessage(p, "&7You have received kit &c" + name + "&7.");
	} else if (nextUse < 1L) {
	    MessageManager.sendMessage(p, "&cYou have already used this kit &conce&7.");
	} else {
	    MessageManager.sendMessage(p, "&7You can use this kit in &c" + DateUtil.formatDateDiff(nextUse) + "&7.");
	}
    }
	...
	if (lastUse == 0L || lastUse > current.getTimeInMillis())
	    ...
	    // If the kit has been used recently, return the next time it can be
	    // used.
