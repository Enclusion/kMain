 ...
 */
public class CommandEco extends BaseCommand {
	...
	if (args.length == 0) {
		...
		MessageManager.sendMessage(p,
			"&b" + prof.getName() + "&7: &aE&7: &c" + prof.getEmeraldsSold() + " &bD&7: &c"
				+ prof.getDiamondsSold() + " &6G&7: &c" + prof.getGoldSold() + " &7I: &c"
				+ prof.getIronSold());
	    }
		...
		try {
		    ...
		    MessageManager.sendMessage(p,
			    "&7You have given &c$" + eco.formatDouble(amount) + " &7to &c" + prof.getName() + "&7.");
		...
		try {
		    ...
		    prof.setBalance(prof.getBalance() - amount);
		    MessageManager.sendMessage(p,
			    "&7You have taken &c$" + eco.formatDouble(amount) + " &7from &c" + prof.getName() + "&7.");
		} catch (NumberFormatException e) {
		    MessageManager.sendMessage(p, "&cThe amount must be a number.");
		}
		...
		try {
		    ...
		    MessageManager.sendMessage(p,
			    "&7cYou have set the balance of &c" + prof.getName() + " &7to &c" + amount + "&7.");
		} catch (NumberFormatException e) {
