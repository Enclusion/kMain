...
public class Command_balance extends BaseCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            MessageManager.sendMessage(p, "&7Balance: &c" + eco.formatDouble(pm.getProfile(p.getUniqueId()).getBalance()));
            return;
        }
            ...
            if (args[0].equalsIgnoreCase("top")) {
                new BukkitRunnable() {
                    public void run() {
                        ...
                        for (Object e : a) {
                            if (topten <= 9) {
                                MessageManager.sendMessage(p, "&7" + (topten + 1) + " - &b" + ((Map.Entry<String, Double>) e).getKey() + "&7: &c" + eco.formatDouble(((Map.Entry<String, Double>) e).getValue()));
                            }
                ...
                MessageManager.sendMessage(p, "&c" + tprof.getName() + " has a balance of &c" + eco.formatDouble(tprof.getBalance()) + "&7.");
            }
        }
    }
}
