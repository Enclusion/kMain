    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            if (km.getKits().isEmpty()) {
                MessageManager.message(p, "&7There are no kits available.");
                return;
            }
            ...
            MessageManager.message(p, "&7Kits Available: \n" + sb.toString());
        } else if (args.length == 1) {
            if (km.getKit(args[0]) == null) {
                MessageManager.message(p, "&7There is no kit &c" + args[0] + "&7.");
                return;
            }
            ...
            if (!p.hasPermission("kits." + k.getName()) && !p.hasPermission("kits.*")) {
                MessageManager.message(p, "&cYou are not allowed to do this.");
                return;
            }
