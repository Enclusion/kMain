...
/**
 * Created by Calvin on 4/27/2015. Project: Legacy
 */
public class SpawnListeners implements Listener {
	...
	if (e.getMessage().toLowerCase().contains("/team sethq") || e.getMessage().toLowerCase().contains("/team shq")
		|| e.getMessage().toLowerCase().contains("/t sethq") || e.getMessage().toLowerCase().contains("/t shq")
	    ...
	    if (e.getMessage().toLowerCase().contains("/tpa") || e.getMessage().toLowerCase().contains("/tpaccept")
	...
	CommandHud.getDisplayList().add(p.getUniqueId());

	PlayerUtility.updateScoreboard(p);
	//
	// main.getTeamTagManager().initPlayer(p);
	// main.getTeamTagManager().sendTeamsToPlayer(p);
	// main.getTeamTagManager().reloadPlayer(p);
    }
	...
	main.getTeamTagManager().getTeamMap().remove(e.getPlayer().getName());
    }
	...
	if (sm.getSpawn().isInStoneRadius(e.getLocation())
		...
		&& e.getEntity().getType() != EntityType.VILLAGER && e.getEntity() instanceof Monster) {
	    e.setCancelled(true);
	    return;
	}
	...
	if (e.getBlock().getLocation().getY() <= sm.getSpawn().getStoneHeight()
		&& sm.getSpawn().isInStoneRadius(e.getBlock().getLocation())
		|| e.getBlock().getLocation().getY() >= sm.getSpawn().getStoneHeight() + 15
			&& sm.getSpawn().isInStoneRadius(e.getBlock().getLocation())) {
	    e.setCancelled(true);
	    return;
	}
	...
	if (sm.getSpawn().isInStoneRadius(e.getBlock().getLocation())
		&& !sm.getSpawn().isInSpawnRadius(e.getBlock().getLocation())) {
	...
	if (sm.getSpawn().isInStoneRadius(e.getBlock().getLocation())
		&& !sm.getSpawn().isInSpawnRadius(e.getBlock().getLocation())) {
	    if (e.getBlock().getType() == Material.BED || e.getBlock().getType() == Material.SAND
		    || e.getBlock().getType() == Material.GRAVEL || e.getBlock().getType() == Material.IRON_DOOR_BLOCK
		    || e.getBlock().getType() == Material.IRON_DOOR || e.getBlock().getType() == Material.WOOD_DOOR
		    || e.getBlock().getType() == Material.WOODEN_DOOR || e.getBlock().getType() == Material.BED_BLOCK) {
		e.setCancelled(true);
	    }

	    butils.onBlockPlace(new BlockPlaceEvent(e.getBlock(), e.getBlockReplacedState(), e.getBlockAgainst(),
		    e.getItemInHand(), e.getPlayer(), true), 600 * 20);
	}
    }
	...
	if (e.hasItem()) {
	    if (e.getItem().getType() == Material.BUCKET || e.getItem().getType() == Material.LAVA_BUCKET
	...
	// Location standBlock =
	// p.getWorld().getBlockAt(p.getLocation().add(0.0D, -0.1D,
	// 0.0D)).getLocation();
	// int xblock = 0;
	// double xvel = 0.0D;
	// int yblock = -1;
	...
	// while (standBlock.getBlock().getLocation().add(xblock, -1.0D,
	// 0.0D).getBlock().getType().equals(Material.SPONGE)) {
	// xblock--;
	// xvel += 0.8D;
	// }
	//
	// while (standBlock.getBlock().getLocation().add(0.0D, yblock,
	// 0.0D).getBlock().getType().equals(Material.SPONGE)) {
	// yblock--;
	// yvel += 1.0D;
	// }
	//
	// while (standBlock.getBlock().getLocation().add(0.0D, -1.0D,
	// zblock).getBlock().getType().equals(Material.SPONGE)) {
	// zblock--;
	// zvel += 0.8D;
	// }
	//
	...
	// }
	//
	// if (standBlock.getBlock().getLocation().add(0.0D, -1.0D,
	// 0.0D).getBlock().getType().equals(Material.SPONGE)) {
	...
	if (sm.getSpawn().isInStoneRadius(e.getBlock().getLocation()) && e.getItem().getType() == Material.LAVA_BUCKET
		|| e.getItem().getType() == Material.WATER_BUCKET) {
	    e.setCancelled(true);
	}
    }
